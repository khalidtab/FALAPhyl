...
option_list = list(
  make_option(c("-i", "--input"), type="character", default=NULL, help="PCoA output from Qiime", metavar="Input PCoA output from Qiime"),
...
ProportionsExplained = suppressMessages(readLines(PCoA,n=5)) %>% # read til the line that has the explained proportions
...
coordinates = suppressMessages(read_tsv(PCoA,skip=9,skip_empty_rows = TRUE)) %>% head(.,n=-2) # read til the line that has the explained proportions
...
myplot = myplot + labs(x=paste0("PCoA1 - variance explained: ",round(ProportionsExplained[1],2),"%"),y=paste0("PCoA2 - variance explained: ",round(ProportionsExplained[2],2),"%"))
...
ggsave(filename=paste0(output,"withnamesprobDF.svg"),plot=p4)
